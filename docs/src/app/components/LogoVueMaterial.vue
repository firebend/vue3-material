...
<script>
import MdSvgLoader from 'vue-material/components/MdSvgLoader/MdSvgLoader.vue'
  ...
  .logo-vue-material {
    ...
    @keyframes first-cycle {
      @each $color, $item in $hue1 {
        #{$counter}% {
          fill: palette.md-get-palette-color($color, A200);
        }
      ...
      100% {
        fill: palette.md-get-palette-color(orange, A200);
      }
    ...
    @keyframes last-cycle {
      ...
      100% {
        fill: palette.md-get-palette-color(lime, A200);
      }
    }
    ...
    $transition-square: .6s .5s variables.$md-transition-stand-timing;
